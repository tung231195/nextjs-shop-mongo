...
import { useRouter } from "next/router";
  ...
  const handRemoveItem = (itemId: string) => {
    const listCart = cartItems.filter((item) => item.product != itemId);
  ...
  const {user} = useAuth()
    ...
    //effect 
    React.useEffect(() => {
      const CartItemsStore = getCartItemsStoreData();
      const parseCartItemsStore = JSON.parse(CartItemsStore.cartItems as string) ?? []
      if(userId) {
        ...
        >
          {cartItems &&
            cartItems.map((item) => {
              ...
              return (
                <Box key={`minicart-${item.product}`} sx={{ display: "flex" }}>
                  ...
                  <Box>
                    <Icon
                      icon="mdi:remove"
                      onClick={() => handRemoveItem(item.product)}
                    />
                  </Box>
                </Box>
              );
