...
import { getAllRoles } from "src/service/auth";
import { AppDispatch, RootState } from "src/stores";
import { deleteRoleAction, getAllRoleByIdAction, getRoleAction, updateRoleAction } from "src/stores/apps/role/roleAction";
...
const MANAGE_USER = ()=> {
  ...
  /**handEditRole */
  const handEditRole = (e: MouseEvent<HTMLButtonElement, MouseEvent>,params:GridRenderCellParams<any, any, any, GridTreeNodeWithRender>) => {
  ...
  const allroles = useSelector((state:RootState) => state.roleSlide.roles)
   console.log('fetch all role',allroles,LIST_DATA_PERMISION);
  return (
    <Box sx={{height:"100%",maxWidth:"100% !important"}}>
        <Button onClick={handleCreateRole}>{t("create_role")}</Button>
     <Grid container >
          <Grid item xs={5}>
            {allroles && <CustomDataTable onRowClick={handleRowClick}   getRowId={getRowId} rows={allroles} columns={columns}  />}
          </Grid>
          <Grid item xs={7}>
              <Button onClick={handleUpdateRole}>{t("update_role")}</Button>
              <PermissionTable checkedPermission={checkedPermission} setCheckedPermisson={setCheckedPermisson} /> 
          </Grid>
     </Grid> 
