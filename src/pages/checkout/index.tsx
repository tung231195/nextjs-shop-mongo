...
const CheckoutPage = () => {
  ...
  const [openModalNewEditAdress, setOpenModalNewEditAdress] = useState({open:false,_id:""});
  //** router */
  ...
  const paymentList = useSelector((state:RootState)=>state.paymentSlide.payments)
  let orderObjItems = [];

  useEffect(() => {
    if (user && user._id) {
      orderObjItems = objCarts && objCarts[user?._id];
      let orders = orderObjItems.filter((item: ProductItem) =>
  ...
  console.log("delivery", deliverySelect, delivery);
  return (
    <Box sx={{width:"1200px", margin:"0 auto"}}>
      <Box sx={{ display: "flex", flexDirection:"column" }}>
        ...
        <Box sx={{display:"flex", alignItems:"center"}}>
          ...
          <Button onClick={() => setOpenModalAdress(true)}>Change Address</Button>
        </Box>
        ...
        {openModalAdress && (
          ...
          >
            <EditAddressForm user={user} handlAddNewAddress={handlAddNewAddress} hanldeUpdateAdress={hanldeUpdateAdress}/>
          </CustomModal>
        )}
        ...
        {openModalNewEditAdress.open &&
          ...
          >
            <NewEditAdressForm openModalNewEditAdress={openModalNewEditAdress}   user={user} handleBackAdress={handleBackAdress} />
          </CustomModal>
        }
      </Box>
      <Box>
        ...
        <Grid container>
          ...
          <Grid item md={6}>
            <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
              <Typography>Delivery Method: {deliveryMethodSelected ?deliveryMethodSelected.name: deliveryMethodSelected}</Typography>
              <Button
                onClick={() => {
      ...
      <Box>
        <Grid container>
          <Grid item md={6}>
            <Box>
              <Typography>Payment Method: {payment ?payment.name: payment} </Typography>
              {showPayments && 
               <FormControl>
                ...
                >
                  {paymentList && paymentList.map((p) => {
                    return (<React.Fragment key={p._id}>
                              <FormControlLabel onChange={()=> setPayment(p)} value={p._id} control={<Radio />} label={p.name} />
                          </React.Fragment>)
                  })}
                </RadioGroup>
              </FormControl>
